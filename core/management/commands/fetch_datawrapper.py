...
class Command(BaseCommand):
    help = 'Fragt alle 2 Minuten die Datawrapper-API ab und speichert neue Grafiken ab dem 01.01.2025 in der Datenbank.'
    ...
    def handle(self, *args, **kwargs):
        ...
        while True:
            ...
            # Verarbeite alle gefundenen Grafiken
            for chart in all_charts:
                chart_id = chart.get('id')
                if not chart_id:
                    continue

                # Prüfe das Erstellungsdatum
                created_at = chart.get('createdAt')
                if created_at:
                    try:
                        # Erhalte ein offset-aware Datum
                        created_date = datetime.fromisoformat(created_at.replace('Z', '+00:00'))
                        if created_date < start_date:
                            self.stdout.write(f"Chart {chart_id} wurde vor 2025 erstellt, überspringe...")
                            continue
                    except Exception as e:
                        self.stderr.write(f'Fehler beim Parsen des Erstellungsdatums für {chart_id}: {e}')
                        continue
